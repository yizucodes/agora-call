...
/** Subset required for Agora Real-Time STT REST routes (no OpenAI). */
export const STT_SERVER_ENV_KEYS = [
  'AGORA_APP_ID',
  'AGORA_APP_CERTIFICATE',
  'AGORA_CUSTOMER_KEY',
  'AGORA_CUSTOMER_SECRET',
...
export type SttServerEnvKey = (typeof STT_SERVER_ENV_KEYS)[number]
...
/**
 * Env vars for STT API routes only. Use this from `/api/stt/*` so OpenAI is not required.
 */
export function requireSttServerEnv(): SttServerEnv {
  const missing: string[] = []
